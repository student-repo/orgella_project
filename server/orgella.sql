create database if not exists orgella;
  ...
  create table if not exists order_details(ID int(5) not null auto_increment, OrderID int(5) not null, Quantity int(5) not null, OfferID int(5) not null, UnitPrice int(5) not null, ShipmentType varchar(30) not null, primary key(ID), constraint order_details_key foreign key (OrderID) references orders(OrderID) on delete cascade);
  ...
  create table if not exists shipment(OfferID int(5) not null, ShipmentType varchar(30) not null, Price int(5) not null, Description varchar(100), MaxInParcel int(5) not null, constraint shipment_key foreign key (OfferID) references offers(OfferID) on delete cascade);
